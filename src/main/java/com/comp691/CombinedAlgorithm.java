...
public class CombinedAlgorithm {
    ...
    public int combinedAlg(int k, List<Integer> pageRequestSequence, List<Integer> predictedHSeq, double thr) {
        ...
        for (int index = 0; index < n; index++) {
            ...
            // Blind Oracle Algorithm Implementation
            ...
            // We have current f1 and f2 so far,

            // For the first k requests, f1 and f2 will be the same, so ALG maintains LRU
            if (CombinedALG_cache.size() < k) {
                r_pageFault += 1;
                CombinedALG_cache.add(requestedPage);
            }

            // for requests i >= k+1, it will perform the following
            if ((double) (f1) > (double) ((1 + thr) * f2) && ALG == CURRENT_ALGORITHM.LEAST_RECENTLY_USED) {
                // We switch from LRU to Blind Oracle algorithm as f1 > (1+thr)f2
                ALG = CURRENT_ALGORITHM.BLIND_ORACLE;
                r_pageFault += BO_cache.size();

                // The combined alg cache is flushed and populated with Blind oracle's cache
                // content
                CombinedALG_cache.clear();
            ...
            // No change
                ...
                if(removedPage == -1) {
                    // No page was evicted, no need to update the cache

                }

                else{
                    // page has been removed and it is removedPage
                    CombinedALG_cache.remove(removedPage);
                    CombinedALG_cache.add(requestedPage);
                    r_pageFault += 1;
                }
